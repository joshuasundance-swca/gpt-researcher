...
class WriterAgent:
    ...
    async def write_sections(self, research_state: dict):
        ...
        response = await acall_model(prompt, task.get("model"), max_retries=2, response_format='json')
...
"""
        ...
        response = await acall_model(prompt, task.get("model"), response_format='json')
        return {"headers": json.loads(response)}

    async def run(self, research_state: dict):
        await stream_agent_output(self.websocket, f"Writing final research report based on research data...", agent="WRITER")
        research_layout_content = await self.write_sections(research_state)

        if research_state.get("task").get("verbose"):
            await stream_agent_output(self.websocket, research_layout_content, agent="WRITER")

        headers = self.get_headers(research_state)
        if research_state.get("task").get("follow_guidelines"):
            await stream_agent_output(self.websocket, "Rewriting layout based on guidelines...", agent="WRITER")
            headers_resp = await self.revise_headers(task=research_state.get("task"), headers=headers)
            headers = headers_resp.get("headers")
